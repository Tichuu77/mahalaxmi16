...
/* ─────────────────────────────────────────
   Featured Card — landscape on desktop,
   stacked on mobile
───────────────────────────────────────── */
function FeaturedCard({ project }: { project: Project }) {
  ...
  const handleWhatsApp = () => {
    const msg = `Hi, I'm interested in "${project.title}" at ${project.location}. Could you share more details?`
    window.open(`https://wa.me/918999537942?text=${encodeURIComponent(msg)}`, "_blank")
  }
      ...
      <div className="flex flex-col lg:grid lg:grid-cols-[1.5fr_1fr]">
        ...
        <div className="p-6 sm:p-8 lg:p-10 flex flex-col justify-between">
          ...
          <button
            ...
            className="group flex items-center justify-center gap-2 font-bold text-sm px-6 py-3.5 rounded-xl text-white transition-all duration-300 hover:scale-[1.02] active:scale-95"
            style={{
...
function ProjectCard({ project }: { project: Project }) {
  ...
  const handleWhatsApp = () => {
    const msg = `Hi, I'm interested in "${project.title}" at ${project.location}. Could you share more details?`
    window.open(`https://wa.me/919579889398?text=${encodeURIComponent(msg)}`, "_blank")
  }
  ...
  return (
    <div
      ...
      style={{ background: "#fff", border: "1px solid rgba(48,83,74,0.1)", boxShadow: "0 2px 12px rgba(48,83,74,0.06)" }}
    >
      {/* Image */}
...
export function ProjectsSection() {
  ...
  return (
    ...
    >
      ...
      {/* ── Project Cards ── */}
      <div
        className={`max-w-[1400px] mx-auto pl-8 pr-6 sm:pl-16 sm:pr-10 lg:px-24 pb-20 relative z-10 transition-all duration-700 delay-100 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
      >
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            <div className="text-5xl mb-4">🏗️</div>
            <p className="text-base" style={{ color: "#aaa", fontFamily: "'Inter', sans-serif" }}>

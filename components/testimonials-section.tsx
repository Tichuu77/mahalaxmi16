...
// Precomputed star arrays — avoid Array.from({length}) on every render
...
// Rating summary strip (same on both desktop + mobile)
const RatingStrip = memo(({ starSize = 11, fontSize = "1.8rem" }: { starSize?: number; fontSize?: string }) => (
  ...
  >
    <div>
      <div className="font-bold leading-none" style={{ fontFamily: "'Poppins', sans-serif", fontSize, color: "#C9862b" }}>5.0</div>
...
// Active testimonial card (shared between desktop + mobile)
const ActiveCard = memo(({ t, quoteSize, contentSize }: { t: typeof testimonials[number]; quoteSize: number; contentSize: string }) => (
  <div className="rounded-2xl p-6 lg:p-8 relative overflow-hidden" style={{ background: "#30534A" }}>
    <Quote size={quoteSize} className="absolute -top-4 -right-4 opacity-[0.06]" style={{ color: "#fff" }} />
    <div className="mb-4 lg:mb-5">
      <Stars count={t.rating} size={13} />
    </div>
    <p className="leading-relaxed mb-6 lg:mb-8 italic" style={{ color: "rgba(255,255,255,0.85)", fontFamily: "'Inter', sans-serif", fontSize: contentSize }}>
      "{t.content}"
    </p>
    <div className="flex items-center gap-3">
      ...
      <div>
        ...
        <p className="text-xs" style={{ color: "#C9862b", fontFamily: "'Inter', sans-serif" }}>{t.location}</p>
      </div>
    </div>
  </div>
))
    ...
    <div className="flex gap-2">
      {testimonials.map((_, i) => (
        <button key={i} onClick={() => onDot(i)}
          ...
          style={{ width: i === current ? "2rem" : "0.5rem", background: i === current ? "#C9862b" : "rgba(48,83,74,0.2)" }}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 pl-8 pr-8 sm:pl-16 sm:pr-12 lg:px-24 py-5 relative z-10"
        ...
        <span className="text-[10px] tracking-[0.35em] uppercase font-bold" style={{ color: "#C9862b", fontFamily: "'Poppins', sans-serif" }}>Testimonials</span>
        <span className="flex-1 h-px" style={{ background: "rgba(48,83,74,0.1)" }} />
        <span className="text-[10px] tracking-[0.2em] uppercase font-medium" style={{ color: "rgba(48,83,74,0.35)", fontFamily: "'Inter', sans-serif" }}>17,000+ Happy Families</span>
      </div>
      ...
      <div className="max-w-[1400px] mx-auto pl-8 pr-8 sm:pl-16 sm:pr-12 lg:px-24 pt-12 pb-20 relative z-10">
        ...
        {/* Desktop */}
        <div className={`hidden lg:grid grid-cols-[1fr_1px_1fr] gap-0 items-start transition-all duration-700 delay-100 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
          style={{ willChange: "transform, opacity" }}>
            ...
            {testimonials.map((t, i) => (
              ...
              >
                ...
                <p className="text-xs leading-relaxed"
                  style={{ color: i === current ? "#555" : "#aaa", fontFamily: "'Inter', sans-serif",
        ...
        {/* Mobile */}
        <div className={`lg:hidden transition-all duration-700 delay-100 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
          style={{ willChange: "transform, opacity" }}>
          <div className="mb-5">
